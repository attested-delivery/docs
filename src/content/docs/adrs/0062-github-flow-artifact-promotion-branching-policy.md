---
title: "ADR 0062: GitHub Flow with Artifact-Promotion Branching Policy"
...
# ADR 0062: GitHub Flow with Artifact-Promotion Branching Policy
...
- The governance properties teams associate with git-flow are obtained at other layers in this design: the release "tag-and-ceremony" maps to the change-gate record (ADR 0061 / governance), and blast-radius isolation maps to separate per-environment GitOps control planes — not to branch topology (f_gitflow_branching_model_3, f_gitflow_branching_model_5).
...
2. **The release candidate and the release are the attested digest, not a branch.** Stabilization happens by promoting the digest through INT→CERT→change-gate→PROD (ADR 0056). There is no `release/*` branch in the normal flow.
3. **A hotfix is a short-lived `hotfix/*` branch off `main`, shipped fix-forward.** It carries the *same* attestation requirements as any other change (SLSA provenance, SBOM, signature — ADR 0057/0058) and an expedited approval path (HOTFIX-RUNBOOK); the expedited path compresses *approvals*, not *provenance*. The fix lands on `main` and is promoted as a normal digest.
4. **Versioning:** tag the release commit on `main` with an annotated SemVer git tag, and apply an immutable, informational registry tag to the digest. The digest remains canonical; tags never drive promotion (ADR 0056 / digest-canonical invariant).
...
- GitHub Flow assumes fast, reliable CI and disciplined small PRs; weak CI makes a single `main` riskier (mitigated by the local-first + multi-gate pipeline, ADR 0059 and validation loop).
...
- **Enables / relies on:** digest-canonical, attestation-preserving promotion (ADR 0056) — the reason a release candidate need not be a branch.
- **Related:** ADR 0061 (DORA — "deployment" defined at the prod digest promotion, not a branch merge), ADR 0057/0058 (attestation requirements that a hotfix branch must still satisfy), HOTFIX-RUNBOOK (the fix-forward procedure).
