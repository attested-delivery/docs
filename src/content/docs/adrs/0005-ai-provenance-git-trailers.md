...
# ADR 0005: AI Provenance via git-creep Trailers Injected into SLSA externalParameters
...
1. **DORA 2025 shows AI degrades stability while improving throughput.** Faros telemetry (22k devs) shows +54% bugs/dev and +242.7% incidents/PR for AI-assisted commits (f_dora_metrics_9). Segmenting DORA metrics by AI authorship mode (ADR 0006) requires per-commit provenance data to exist.
...
2. **Analytics axis**: the `AI-Authoring-Mode` value and associated confidence weight feed the DORA cohort segmentation defined in ADR 0006.
...
The creep library operates in scope-limited parse-and-record mode; analytics, estimators, and DORA overlays are handled by ADR 0006 tooling, not creep itself (f_git_creep_ai_provenance_17).
...
- Update `slsa-sign.yml` (ADR 0002) to append `externalParameters.ai.*` fields from the commit trailers before signing.
...
- Per-commit AI-authorship metadata exists for DORA cohort segmentation (ADR 0006) and SLSA `externalParameters` enrichment.
...
- `externalParameters.ai.*` field injection into the SLSA predicate requires coordination with ADR 0002's signing workflow to ensure the fields are populated before signing, not after.
...
- **Depends on:** ADR 0002 (SLSA provenance signing workflow — AI fields injected into `externalParameters` before signing).
- **Feeds into:** ADR 0006 (DORA instrumentation — `AI-Authoring-Mode` weight is the AI-cohort segmentation input).
- **Related:** ADR 0004 (admission-time verification operates on the SLSA predicate that embeds AI provenance fields).
