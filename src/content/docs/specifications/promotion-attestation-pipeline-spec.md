...
**Date:** 2026-06-01
**Status:** DRAFT — encodes design decisions ADR-0056–0063
...
**Note:** Any deprecated IaC scanner (`tfsec` or similar) must be replaced — **excluding** `trivy-action` (CVE-2026-33634, CRITICAL; f_supply_chain_security_sbom tooling warning; ADR-0063 mandates Syft+Grype and digest-pinned security tooling).
...
**ADR refs:** ADR-0057 (SLSA L3), ADR-0058 (CycloneDX 1.6), ADR-0060 (AI-provenance), ADR-0063 (Syft+Grype, NOT trivy-action).
...
**Tooling constraint (ADR-0063):** `trivy-action` was supply-chain-compromised March 2026 (CVE-2026-33634, CRITICAL — 76/77 tags poisoned, exfiltrating cloud credentials from CI). Standardize on **Syft** (SBOM) + **Grype** (scan). Pin all security tooling by digest, never mutable tag.
...
**ADR refs:** ADR-0056 (carry referrers — closes crane-cp orphaning gap).

**The crane-cp gap (ADR-0056):** A naive `crane cp <src>@<digest> <dst>:<tag>` + `crane digest` verify copies manifest+layers only — OCI referrers (signatures, SLSA, SBOM) are **not carried** (f_artifact_attestation_promotion_3). Fix options in preference order:
...
**ADR refs:** ADR-0056 (referrer-aware copy applied).
...
**ADR refs:** ADR-0059 (Kyverno admission-time attestation verification). Note: OPA Gatekeeper, if present for pod security standards, is supplemented — not replaced — by Kyverno's `ImageValidatingPolicy`; the two policies serve different purposes.
...
sequenceDiagram
    ...
    Step->>ECR_N: cosign copy --only=sig,att,sbom<br/>OR oras cp -r<br/>[REQUIRED FIX — ADR-0056]
...
jobs:
  attest:
    ...
    steps:
      ...
      - name: Install Syft (SBOM) — pinned by digest (ADR-0063)
        run: |
          curl -sSfL https://raw.githubusercontent.com/anchore/syft/main/install.sh \
            | sh -s -- -b /usr/local/bin v1.5.0
          # TODO: replace URL with digest-pinned download in production

      - name: Install Grype (vuln scan) — pinned by digest (ADR-0063)
        run: |
      ...
      # AI-provenance attest (ADR-0060) — conditional on commit trailers
...
### 4.2 Attestation-Preserving Promotion Step (ADR-0056 fix)
...
### 4.3 Kyverno ImageValidatingPolicy (Admission Gate — ADR-0059)
...
metadata:
  ...
  annotations:
    ...
    policies.kyverno.io/description: >
      ...
      authorized GitHub Actions workflow. Satisfies ADR-0059 / SOC2 CC7.2.
spec:
...
| **crane** | Any (existing) | Manifest inspection, digest lookup only | Do NOT use `crane cp` for attestation-bearing promotions (ADR-0056) |
...
- [ ] **ADR-0056:** Replace `crane cp` with `cosign copy --only=sig,att,sbom` in `promote-int.yml`, `promote-cert.yml`, `promote-prod.yml`
- [ ] **ADR-0056:** Replace post-promotion `crane digest` gate with `cosign verify-attestation --type slsa`
- [ ] **ADR-0059:** Deploy Kyverno `ImageValidatingPolicy` in `Audit` mode across all EKS clusters
- [ ] **ADR-0063:** Audit all workflows for `trivy-action` references; replace with Syft+Grype pinned by digest
- [ ] **ADR-0063:** Audit all security tool references for mutable tags (`@master`, `@latest`); pin by digest

### Phase 1 — Declare and standardize (P1, ~2 weeks)

- [ ] **ADR-0057:** Implement isolated reusable signing workflow (SLSA L3 pattern); verify with GitHub's SLSA verifier
- [ ] **ADR-0058:** Switch SBOM format to CycloneDX 1.6 primary; add SPDX 2.3 export for procurement; attach to digest via OCI referrers
- [ ] **ADR-0062:** Codify GitHub Flow branching policy (short-lived branch off `main`; `hotfix/*` fix-forward); document `release/x.y` long-lived branches as the rare exception for genuinely back-supported releases only; record git-flow as the evaluated-and-rejected alternative
- [ ] **ADR-0059:** Flip Kyverno from `Audit` to `Enforce` in non-prod; validate no false positives over one sprint, then flip in PROD

### Phase 2 — Measure and enrich (P2, ~2 weeks)

- [ ] **ADR-0061:** Instrument five DORA events (see §5); define `deploy` = prod digest promotion; add `gate_ccab_duration_seconds` metric
- [ ] **ADR-0060:** Implement git-creep `prepare-commit-msg` hook (exits 0 on failure, never blocks); add AI-provenance predicate to S2 attest workflow
...
| --- | --- | --- | --- |
| **ADR-0056** | Attestation-preserving digest promotion (cosign copy / oras cp -r; post-promote `cosign verify-attestation`) | P0 | Referrer-aware promotion; close the copy-by-digest orphaning failure mode |
| **ADR-0057** | Declare SLSA Build L3 via isolated reusable signing workflow | P1 | Provenance isolation: L2→L3 upgrade |
| **ADR-0058** | CycloneDX 1.6 SBOM primary + SPDX export, attached via OCI referrers | P1 | Declare format and attach to digest |
| **ADR-0059** | Admission-time attestation verification (Kyverno ImageValidatingPolicy, Deny+Fail) | P0 | Enforce attestation at runtime, not by convention |
| **ADR-0060** | AI-provenance git trailers → SLSA `externalParameters` predicate (git-creep schema) | P2 | AI authorship visible in provenance and DORA cohorts |
| **ADR-0061** | DORA instrumentation (deployment = prod digest promotion; five-event model) | P2 | Define "deployment" precisely; instrument per-gate latency |
| **ADR-0062** | GitHub Flow + artifact-promotion branching policy (git-flow recorded as rejected alternative) | P1 | GitHub Flow for continuous delivery; digest is the RC, not a release branch |
| **ADR-0063** | Syft+Grype; pin all security tooling by digest; trivy-action banned (CVE-2026-33634) | P0 | Supply-chain-safe scanner baseline |
